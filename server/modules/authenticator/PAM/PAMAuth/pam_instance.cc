...
PamInstance* PamInstance::create(char** options)
{
    ...
    const string pam_db_fname = string(get_cachedir()) + "pam_db";
    ...
    if (sqlite3_open_v2(pam_db_fname.c_str(), &dbhandle, db_flags, NULL) != SQLITE_OK)
    {
        ...
        else
        {
            ...
            MXB_ERROR(SQLITE_OPEN_OOM, pam_db_fname.c_str());
        }
        error = true;
    }
